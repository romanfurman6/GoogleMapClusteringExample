...
static const NSUInteger kGMUMinClusterSize = 3; //4
...
static const float kGMUMaxClusterZoom = 18; //20
...
- (void)renderCluster:(id<GMUCluster>)cluster animated:(BOOL)animated {
  ...
  if ([self shouldRenderAsCluster:cluster atZoom:zoom]) {
    ...
    GMSMarker *marker = [self markerWithPosition:cluster.position
                                     ...
                                     clusterIcon:icon
                                        animated:animated];
    marker.groundAnchor = CGPointMake(37.5 / icon.size.width, 1.0);
    ...
    for (id<GMUClusterItem> item in cluster.items) {
      ...
      GMSMarker *marker = [self markerWithPosition:item.position
                                              from:fromPosition
                                          userData:item
                                       clusterIcon:item.image
                                          animated:shouldAnimate];
      marker.tracksInfoWindowChanges = YES;
      marker.infoWindowAnchor = CGPointMake(25.0 / item.image.size.width, (25.0 / item.image.size.width));
      marker.groundAnchor = CGPointMake(25.0 / item.image.size.width, 1.0);
      [_markers addObject:marker];
...
- (GMSMarker *)markerWithPosition:(CLLocationCoordinate2D)position
  ...
  if (clusterIcon != nil) {
    ...
    marker.groundAnchor = CGPointMake(37.5 / clusterIcon.size.width, 1.0);
  }
